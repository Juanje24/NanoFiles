...
public class NFController {
	...
	private static final byte SERVING_BG=2;
	/*
	...
	public void processCommand() {
		...
		case NFCommands.COM_BGSERVE:
			...
			if (serverRunning) {
				controllerDir.registerFileIP(controllerPeer.getServerIP()); //Registrar la ip en la que sirve -> Mejora downloadfrom por nick
	...
	public boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_LOGOUT:
			if (currentState!=LOGGED_IN) {
				commandAllowed=false;
				System.err.println("* You cannot logout from the directory");
			}
			break;
		case NFCommands.COM_QUIT:
			if (currentState != LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot quit before logging out.");
			}
			break;
		case NFCommands.COM_USERLIST:
			if (currentState != LOGGED_IN && currentState!=SERVING_BG) {
				commandAllowed = false;
				System.err.println("* You cannot get the userlist before logging in.");
			}
			break;
		case NFCommands.COM_BGSERVE:
			if(currentState==SERVING_BG) {
				commandAllowed=false;
				System.err.println("You already have a server in bg.");
			}
			else if (currentState != LOGGED_IN) {
				commandAllowed = false;
				System.err.println("* You cannot serve in bg before logging in.");
			}
			break;	
		case NFCommands.COM_STOP_SERVER:
			if (currentState != SERVING_BG) {
				commandAllowed = false;
				System.err.println("* You cannot stop the server if it has not been started.");
			}
			break;
		default:

...
public class NFControllerLogicDir {
	...
	protected boolean getAndPrintUserList() {
		...
		boolean result = false;
		String[][] list=null;  
		list=directoryConnector.getUserList();
		if (list!=null) {
			result=true;
			System.out.print("Userlist: ");
			for (String name : list[0]) {
				System.out.println(name);
			}
			System.out.println("Servers: ");
			for (String name: list[1]) {
				if(name==null) {
	...
	public boolean registerFileIP(String serverIP) {
		/*
		 * TODO: Darse de alta en el directorio como servidor. Comunicarse con el
		 * directorio (a través del directoryConnector) para enviar la dirección IP
		 * en el que escucha el servidor de ficheros que habremos arrancado
		 * previamente. Se debe enviar la clave de sesión para identificarse. Devolver
		 * éxito/fracaso de la operación.
		 */
		boolean result = false;
		if(directoryConnector!=null) {
			result= directoryConnector.registerServerIP(serverIP);			
		}
		else {
			return false;
		}
	...
	public boolean unregisterFileServer() {
		...
		boolean result = false;
		if(directoryConnector!=null) {
			result= directoryConnector.unregisterServerPort(directoryConnector.getSessionKey());			
		}
		else {
			return false;
		}

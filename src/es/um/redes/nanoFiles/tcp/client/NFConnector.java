...
public class NFConnector {
	...
	public NFConnector(InetSocketAddress fserverAddr) throws UnknownHostException, IOException {
		...
		System.out.println("Connected to... "+serverAddr.getAddress()+":"+serverAddr.getPort());
	...
	public boolean downloadFile(String targetFileHashSubstr, File file) throws IOException {
		 ...
		 */
		PeerMessage msjSent= new PeerMessage(PeerMessageOps.OPCODE_DOWNLOAD,(byte)targetFileHashSubstr.length(),targetFileHashSubstr);
		msjSent.writeMessageToOutputStream(dos);
		 ...
		 */
		PeerMessage msjRecvd=PeerMessage.readMessageFromInputStream(dis); 
		byte op=msjRecvd.getOpcode();
		switch(op) {
			case PeerMessageOps.OPCODE_DOWNLOAD_OK:{
				...
				if (hash.equals(msjRecvd.getFileHash())){
					downloaded=true;
				}
